...
def follow_redirects(url)
  Open3.capture2(
    "curl", "--silent", "--head", "--location", "--globoff", "--output", File::NULL,
...
def clean_url(url)
  # Amazon
  if %r{^https://(www|smile)?\.?amazon\.}.match?(url)
    # For a search page, remove everything after the search query
    return url.sub(%r{&.*}, "") if %r{^https://.*?/s}.match?(url)
